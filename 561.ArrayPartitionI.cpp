...
/*
    ...
    Example 2:
        ...
        Output: 9
        Explanation:
            The optimal pairing is (2, 1), (2, 5), (6, 6). min(2, 1) +
...
/*
    每次选出一个尽可能大的数和一个尽可能比这个数还大的数即可。等价于每次选出
    当前所有数中次大的那个数，然后将当前所有数中最大和次大的数移除。
 */
...
int main()
{
    ...
    nums = { 1,4,3,2 };
    assert(4 == s.arrayPairSum(nums));

    nums = { 6,2,6,5,1,2 };
    assert(9 == s.arrayPairSum(nums));
